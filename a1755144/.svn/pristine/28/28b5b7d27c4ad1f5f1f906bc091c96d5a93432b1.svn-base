...
public class Insertion extends Sort{
	...
	public int [] sortStringByIndex(String [] arr){
		//creat a index array for saving how the algorithm sort the source array
		int [] index = this.getIndex(arr.length);
		...
		for(int i = 1; i < arr.length; i++){
			...
			int keyIndex = index[i];
			int j = i - 1;
			//compare the two strings by the built-in method "compareTo" 
			while( j >= 0 && arr[j].compareTo(key)>0){
				if(arr[j].compareTo(key)>0){
					arr[j+1] = arr[j];
					//alter the index array as the original array do
					index[j+1] = index[j];
					j--;
				}
			}
