# all the three bonus questions are solved
...
while True:
  ...
  # handling the situation that the URI contains a port number
  resourceParts0 = resourceParts[0].split(':', 1)
  # set the origin port number default to 80
  originPort = 80
  # if ':' exists, that indicates a port number follows
  if len(resourceParts0) == 2:
      hostname = resourceParts0[0]
      originPort = int(resourceParts0[1])
  ...
  try:
    ...
    cacheResponse = ''.join(outputdata)

    # analyze the cached HTTP response header
    attrs = dict()
    ...
    # get the value of 'Expires' tag
    date = attrs.get('Expires')

    # initially set the expired flag to false
    expired = False

    try:
        date = eut.parsedate(date)
    except:
        # parse error set expired flag to true
        expired = True

    # the following code tell whether it expires or not,
    # if it expires, set the fileExists to false and raise an IOError
    # to force the program to go through the same process as cache doesn't
    # exists

    # if fall to parse, the cached file should be expired
    if date is None or expired:
        fileExists = False
        raise IOError
    else:
        expiredDate = datetime(*date[:6])
        now = datetime.now()

        # current time > expiration time also indicates the cached file is expired
        if now > expiredDate:
            fileExists = False
            raise IOError
  ...
  # Error handling for file not found in cache
    ...
    if fileExists:
      ...
      try:
        ...
        # ~~~~ INSERT CODE ~~~~
        res = originServerSocket.recv(BUFFER_SIZE)
        # ~~~~ END CODE INSERT ~~~~
