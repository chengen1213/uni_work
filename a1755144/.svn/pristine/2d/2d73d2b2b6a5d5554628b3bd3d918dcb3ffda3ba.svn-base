...
# it assumes this directory is SOMEHWERE/lib
...
case $1 in
add)
	tokeniser="../lib/working-tokeniser"
	tokeniser_path="lib/working-tokeniser"
...
setpadding "cat ${TESTDIR}ExplsSquare-SquareGame.jack"
...
setpadding "diff - ${TESTDIR}ExplsSquare-SquareGame.jack.tokens"
...
if [ "${regenerate}" == "ALL" ] ; then
	...
	for i in * ; do
		input="${i%%.tokens}"
		if [ "${input}" == "${i}" -a ! -f "${input}.tokens" ] ; then
