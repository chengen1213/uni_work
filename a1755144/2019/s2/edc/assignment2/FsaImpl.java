...
//        statePattern = Pattern.compile("[a-zA-Z$_][a-zA-Z0-9$_]*");
    ...
    public void removeState(State s) {
        Set<Transition> transitionsFrom = s.transitionsFrom();
        Set<Transition> transitionsTo = s.transitionsTo();
        for (Transition transition : transitionsFrom) {
            transitions.remove(transition);
            notifyTransitionChanged();
            StateImpl toState = (StateImpl) transition.toState();
            toState.removeTransitionsTo(transition);
        }
        for (Transition transition : transitionsTo) {
            transitions.remove(transition);
            notifyTransitionChanged();
            StateImpl fromState = (StateImpl) transition.fromState();
            fromState.removeTransitionsFrom(transition);
        }
        states.remove(s);
        notifyStateChanged();
    }
    ...
    public Transition newTransition(State fromState, State toState, String eventName) throws IllegalArgumentException {
        if (eventName != null && !isValidTransitionName(eventName))
            throw new IllegalArgumentException();
        if ("?".equals(eventName))
            eventName = null;
    ...
    public boolean isValidTransitionName(String name) {
        if ("?".equals(name))
            return true;
    ...
    private class TransitionImpl implements Transition {
